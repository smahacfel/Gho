...
pub struct CabalDetectorConfig {
    ...
    /// Szerokość okna (jitter) dla skanowania gęstości CU.
    pub cu_window_width: u32,

    /// Minimalny procent holderów w jednym oknie uznawany za nienaturalne skupisko.
    pub min_density_percent: f64,

    /// Relatywny mnożnik względem baseline CU dla uznania klastra za „High Overhead”.
...
impl Default for CabalDetectorConfig {
    fn default() -> Self {
        Self {
            ...
            max_funding_depth: 5,
            cu_window_width: 12_000,
            min_density_percent: 0.20,
...
impl CabalDetectorConfig {
    ...
    pub fn validate_sanity_limits(&self) {
        ...
        if self.cu_window_width == 0 {
            panic!("CRITICAL: CabalDetectorConfig: cu_window_width cannot be 0!");
        }
        if self.min_density_percent <= 0.0 || self.min_density_percent > 1.0 {
            panic!("CRITICAL: CabalDetectorConfig: min_density_percent must be within (0.0, 1.0]! Attempted: {}", self.min_density_percent);
        }
        if self.overhead_multiplier < 1.0 {
            panic!(
                "CRITICAL: CabalDetectorConfig: overhead_multiplier must be >= 1.0! Attempted: {}",
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            RejectReason::BotSwarmDetected {
                ...
                write!(
                    f,
                    "BotSwarmDetected: {:.2}% holders in CU window (avg {}, baseline {}) count {}",
                    density_ratio * 100.0,
                    cu_avg,
                    baseline_cu,
                    count
...
impl TokenContext {
    ...
    /// 5. Silnik Dynamicznej Dystrybucji CU (Sliding Window Density Scan)
    /// Wykrywa nienaturalne skupiska wysokich CU bez sztywnych progów.
    pub fn analyze_dynamic_cu_distribution(
        &self,
        config: &CabalDetectorConfig,
    ) -> Result<(), RejectReason> {
        let mut cu_values: Vec<u32> = self
            .holders
            .iter()
            .filter(|h| h.balance > 0)
            .map(|h| h.buy_tx_cu_usage)
            .filter(|&cu| cu > 0)
            .collect();
        ...
        let baseline_idx = cu_values.len() / 10;
        let baseline_cu = cu_values[baseline_idx];

        let mut max_density_count = 0usize;
        let mut suspicious_cluster_avg = 0u32;
        let total_count = cu_values.len();

        let mut left = 0usize;
        for right in 0..total_count {
            while cu_values[right] - cu_values[left] > config.cu_window_width {
                left += 1;
            }

            let current_count = right - left + 1;
            let current_avg = (cu_values[left] + cu_values[right]) / 2;
            let is_high_overhead =
                current_avg as f64 > (baseline_cu as f64 * config.overhead_multiplier);

            if is_high_overhead && current_count > max_density_count {
                max_density_count = current_count;
                suspicious_cluster_avg = current_avg;
            }
        }

        if max_density_count == 0 {
            return Ok(());
        }

        let density_ratio = max_density_count as f64 / total_count as f64;

        if density_ratio >= config.min_density_percent {
            return Err(RejectReason::BotSwarmDetected {
                count: max_density_count,
                cu_avg: suspicious_cluster_avg,
                baseline_cu,
                density_ratio,
            });
        }
...
impl SecurityEngine {
    ...
    pub fn evaluate_token_security(&self, context: &TokenContext) -> Verdict {
        ...
        if let Err(reason) = context.analyze_dynamic_cu_distribution(&self.config) {
            return Verdict::Reject(reason);
        }
...
mod tests {
    ...
    fn test_sanity_check_panic_cluster_ratio() {
        let _ = CabalDetectorConfig::new(0.25, 2500.0, 4, 0.9, 5, 12_000, 0.20, 1.5);
    }
    ...
    #[test]
    fn test_dynamic_cu_distribution_detects_spike() {
        let mut context = create_test_context(100);
        let config = CabalDetectorConfig::default();

        for i in 0..15 {
            context.holders[i].buy_tx_cu_usage = 8_000;
        }
        for i in 15..100 {
            context.holders[i].buy_tx_cu_usage = 72_000;
        }

        let result = context.analyze_dynamic_cu_distribution(&config);
        match result {
            Err(RejectReason::BotSwarmDetected {
                count,
                baseline_cu,
                density_ratio,
                ..
                ...
                assert_eq!(count, 85);
                assert_eq!(baseline_cu, 8_000);
                assert!(density_ratio > 0.80);
            }
            _ => panic!("Should detect CU density spike, got {:?}", result),
        }
    }

    #[test]
    fn test_dynamic_cu_distribution_no_spike() {
        let mut context = create_test_context(50);
        let config = CabalDetectorConfig::default();

        for (i, holder) in context.holders.iter_mut().enumerate() {
            holder.buy_tx_cu_usage = 5_000 + (i as u32 * 2_500);
        }
    ...
    #[test]
    fn test_dynamic_cu_distribution_insufficient_data() {
        let mut context = create_test_context(4);
        let config = CabalDetectorConfig::default();

        for holder in &mut context.holders {
            holder.buy_tx_cu_usage = 0;
        }
