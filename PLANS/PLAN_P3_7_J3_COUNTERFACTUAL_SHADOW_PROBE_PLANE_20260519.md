...
L1R10 reconciled the `bonding_curve_v2` rows that were being rejected by the
active shadow and counterfactual probe simulation-load prechecks. The result was
...
The remaining blocker is no longer blind `AccountNotFound`, payer handling,
precheck optionality, PDD policy, or threshold tuning. The builder places a
`bonding_curve_v2` account into transaction metas, but that exact pubkey is not
materialized by the local DIAG/MFS truth sources used by the rollout evidence.

### Contract

For shadow/probe simulation, `bonding_curve_v2` is split from generic
`bonding_curve` readiness:
...
If a builder emits `bonding_curve_v2` into simulation account metas, the source
must carry explicit authority diagnostics:
...
The current accepted authority statuses are:
...
Only `authoritative_exact_diag`, `authoritative_mfs`, and
`authoritative_account_state` are simulation-ready. A builder-only
`bonding_curve_v2` is not enough to claim execution readiness.

### Runtime Behavior

L1R11 is fail-closed and additive:

- probe and active shadow rows carry `bonding_curve_v2` authority diagnostics;
- manifest rows carry source authority and mismatch reason for
  `bonding_curve_v2`;
- `builder_only` / non-authoritative `bonding_curve_v2` fails before
  `simulate_buy`;
- skip/failure reason is:

```text
bonding_curve_v2_source_not_authoritative:<status>:<source>:<pubkey>
```

This preserves the L1R5/L1R9 rule: if `bonding_curve_v2` remains in transaction
metas, it is simulation-load required. L1R11 tightens the source side of that
contract: the account must also be authoritative, not merely generated by the
route builder.
...
- change active Gatekeeper policy;
- change PDD/prosperity/HHI thresholds;
- change IWIM;
- remove `bonding_curve_v2` from transaction metas;
- promote V3 to active policy;
- start L2 ablation, collection, Phase B, P2, or live execution.

If a future route can prove that `bonding_curve_v2` is not required, that fix
belongs in the builder account-meta set, not in a precheck exception.
...
- `PASS-A`: authoritative `bonding_curve_v2` is materialized and probe/active
  shadow entries can simulate;
- `PASS-B`: non-authoritative `bonding_curve_v2` is fail-closed before
  simulation with explicit `bonding_curve_v2_source_not_authoritative`.

Failure conditions:

- `builder_only` `bonding_curve_v2` reaches simulation as ready;
