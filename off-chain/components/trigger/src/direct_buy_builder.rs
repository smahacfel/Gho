...
impl DirectBuyBuilder {
    ...
    pub fn build_buy_ix_with_accounts_and_bonding_curve_v2(
        ...
        Self::build_buy_ix_with_accounts_and_remaining(
            payer,
            mint,
            token_program,
            global_config,
    ...
    pub fn build_buy_ix_with_accounts_and_remaining(
        payer: &Pubkey,
        mint: &Pubkey,
        token_program: &Pubkey,
        global_config: Option<Pubkey>,
    ...
    pub fn build_buy_ix_with_accounts_and_remaining_and_creator_vault(
        ...
        // protocol BCV2 tail, while routed exact-sol-in derives the same
        // BCV2 account inside the routed layout.
    ...
    pub fn valid_buyback_remaining_account_count(count: usize) -> bool {
        count == 0 || count == PUMPFUN_BUYBACK_REMAINING_ACCOUNT_COUNT
    }
...
mod tests {
    ...
    fn test_legacy_buy_accounts_match_protocol_bcv2_tail_layout() {
        ...
        let actual: Vec<String> = ix
            ...
            .collect();
        assert_eq!(actual, expected_accounts);
        assert_eq!(&ix.data[0..8], &LEGACY_BUY_DISCRIMINATOR);
        assert_eq!(
            u64::from_le_bytes(ix.data[8..16].try_into().unwrap()),
            9_000_000
