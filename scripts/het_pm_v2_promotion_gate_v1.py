...
* ``validate`` revalidates an existing artifact, including the root conjunction.
...
def load_pr_a_analyzer() -> Any:
    path = Path(__file__).with_name("het_pm_v2_analysis.py")
...
def economic_observations(
    ...
    missed_protection = 0
    counterfactual_executable_paths = 0
    for key in sorted(positions):
        candidates = v2_candidates(comparisons.get(key, []))
        ...
        later_executable_returns = [
            float(return_bps)
            for row, _, return_bps in candidates[1:]
            if int(row["observation_timestamp_ms"]) > candidate_timestamp_ms
        ]
        counterfactual_executable_paths += int(bool(later_executable_returns))
        terminal_pct = terminal.get("executable_gross_return_pct")
        ...
        if not all(
            ...
            for value in (terminal_pct, mfe_bps, entry_ts_ms, observation_ts_ms, absolute_age_ms, entry_raw)
        ):
            continue
        ...
        matched.append(
            {
                ...
                "counterfactual_later_executable_sample_count": len(
                    later_executable_returns
                ),
                "counterfactual_later_executable_max_bps": (
                    max(later_executable_returns) if later_executable_returns else None
                ),
                "counterfactual_later_executable_min_bps": (
                    min(later_executable_returns) if later_executable_returns else None
                ),
    ...
    observed = {
        ...
        "vitality_candidate_positions": sum(
            row["reason"] == "VitalityDecay" for row in matched
        ),
        "mfe_capture_positions": len(capture_deltas),
        "missed_protection_eligible_positions": no_candidate_terminal,
        "counterfactual_executable_path_coverage": ratio(
            counterfactual_executable_paths, len(matched)
        ),
...
def evaluate(
    ...
    for run_id, rows in lifecycle_by_run.items():
        ...
        for key, terminal_group in grouped.items():
            ...
            status = terminal.get("het_pm_v2_comparison_write_status")
            if status != "written" or comparison_id not in comparison_ids:
    ...
    economic_observed, matched = economic_observations(
        positions, comparisons, terminals, replay_by_identity, criteria
    )
    coverage_observed["counterfactual_executable_path_coverage"] = economic_observed[
        "counterfactual_executable_path_coverage"
    ]
