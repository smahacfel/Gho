...
class ScanStats:
    ...
    lifecycle_replaced_records: int = 0
...
def _lifecycle_record_type_priority(record_type: Any) -> int:
...
def _lifecycle_record_sort_key(record: dict[str, Any], source_file_order: int) -> tuple[int, float, int]:
    timestamp_ms = to_float(record.get("timestamp_ms"))
    if timestamp_ms is None:
        timestamp_ms = float("-inf")

    return (
        -_lifecycle_record_type_priority(record.get("record_type")),
        timestamp_ms,
        -source_file_order,
    )
...
def load_unique_lifecycle_records(paths: list[Path], stats: ScanStats) -> dict[str, dict[str, Any]]:
    ...
    by_mint_source: dict[str, int] = {}

    for source_file_order, path in enumerate(paths):
        ...
        for line_no, raw_line in iter_jsonl(path):
            ...
            if mint_id in by_mint:
                ...
                existing_record = by_mint[mint_id]
                existing_source_order = by_mint_source[mint_id]
                if _lifecycle_record_sort_key(merged_ready, source_file_order) <= _lifecycle_record_sort_key(
                    existing_record, existing_source_order
                ):
                    continue
                by_mint[mint_id] = merged_ready
                by_mint_source[mint_id] = source_file_order
                stats.lifecycle_replaced_records += 1
                continue
...
def print_summary(
    ...
    print(f"Lifecycle rekordow wypisanych jako lepsza wersja: {stats.lifecycle_replaced_records}")
